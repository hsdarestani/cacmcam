...
window.ccCheckout=async()=>{try{if(window.CamCamNative?.isBazaarBillingAvailable?.()){CamCamNative.purchasePremium();toast('در حال اتصال به پرداخت کافه‌بازار…');return}const r=await api('/api/billing/checkout',{method:'POST',body:JSON.stringify({plan_code:'premium_monthly'})});location.href=r.redirect_url}catch(e){toast(e.message)}};
window.addEventListener('camcam-bazaar-purchase',async event=>{const detail=event.detail||{};if(detail.error){toast(detail.error);return}try{await api('/api/billing/bazaar/verify',{method:'POST',body:JSON.stringify({purchase_data:detail.purchaseData,signature:detail.signature})});toast('اشتراک پریمیوم فعال شد');setTimeout(()=>location.reload(),700)}catch(e){toast(e.message)}});
