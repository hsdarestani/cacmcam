...
setTorch=async function(on){
  ...
  if(on){try{const cap=t.getCapabilities?.();if(!cap?.torch)return{ok:false,message:'چراغ روی این دوربین از داخل اپ پشتیبانی نمی‌شود.'};await t.applyConstraints({advanced:[{torch:true}]});await sleep2(180);const s=t.getSettings?.();if(s&&Object.prototype.hasOwnProperty.call(s,'torch')&&s.torch!==true)return{ok:false,message:'گوشی فرمان روشن‌کردن چراغ را نپذیرفت.'};torch=true;states();return{ok:true,message:'چراغ روشن شد.'}}catch{return{ok:false,message:'چراغ روشن نشد.'}}}
  try{await t.applyConstraints({advanced:[{torch:false}]});await sleep2(180);const s=t.getSettings?.();if(s&&Object.prototype.hasOwnProperty.call(s,'torch')&&s.torch===false){torch=false;states();return{ok:true,message:'چراغ خاموش شد.'}}}catch{}
  const restarted=await hardRestartTorchOff();return restarted?{ok:true,message:'چراغ خاموش شد و دوربین برای اطمینان دوباره متصل شد.'}:{ok:false,message:'خاموش‌کردن چراغ انجام نشد؛ دوربین را یک‌بار عوض کن.'};
};
...
say=function(text){
  ...
  if(src){try{if(phraseAudio){phraseAudio.pause();phraseAudio.src=''}phraseAudio=new Audio(src+'?v=2');phraseAudio.volume=1;phraseAudio.preload='auto';phraseAudio.addEventListener('ended',restore,{once:true});phraseAudio.addEventListener('error',()=>{restore();try{oldSay(text)}catch{}},{once:true});phraseAudio.play().catch(()=>{restore();try{oldSay(text)}catch{}});setTimeout(restore,5500);return true}catch{restore()}}
  try{const ok=oldSay(text);setTimeout(restore,4500);return !!ok}catch{restore();return false}
};
const oldTelemetry=telemetry;telemetry=async function(){try{await oldTelemetry()}finally{try{const info={};if(window.CamCamNative?.getBatteryInfo)Object.assign(info,JSON.parse(window.CamCamNative.getBatteryInfo()));await req('/api/pet/device/telemetry',{method:'POST',headers:hdr(),body:JSON.stringify({...info,low_power:lowPower,codec:window.codec||null,facing,quality:settings.quality,zoom:Number(zoom)||1,talk_connected:talkPc?.connectionState==='connected'})})}catch{}}};
})();
